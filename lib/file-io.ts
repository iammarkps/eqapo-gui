...
export async function handleExportProfile(
    ...
    const profile = {
        ...
        preamp,
        bands: bands.map(({ filter_type, frequency, gain, q_factor, enabled }) => ({
            filter_type,
            frequency,
            gain,
            q_factor,
...
export function handleImportProfile(
    ...
    input.onchange = async (e) => {
        ...
        try {
            ...
            // Load bands with new IDs (preserve enabled field, default to true if missing)
...
export async function handleExportTxt(
    ...
    // Only export enabled bands to EAPO txt format
    const enabledBands = bands.filter(band => band.enabled);
    enabledBands.forEach((band, index) => {
