...
**EQAPO GUI** is a modern, high-performance GUI for **EqualizerAPO**, built with **Tauri v2** and **Next.js**. It provides a sleek, dark-themed parametric equalizer interface that syncs in real-time with your system audio.

![EQAPO GUI UI](https://via.placeholder.com/800x450?text=EQAPO+GUI+Screenshot)
...
Before running EQAPO GUI, ensure you have the following:
...
```bash
# Clone
git clone https://github.com/your-username/eqapo-gui.git
cd eqapo-gui
...
`src-tauri/target/release/bundle/nsis/EQAPO GUI_x.x.x_x64-setup.exe`

Double-click this file to install EQAPO GUI on your machine.
...
3.  Choose where you want the active config file to live.
    *   **Recommended**: Default (`Documents/EQAPO GUI/live_config.txt`).
...
4.  **Important**: If using the default path, open **EqualizerAPO Configuration Editor** and add this line to your `config.txt`:
    ```
    Include: C:\Users\YOUR_USERNAME\Documents\EQAPO GUI\live_config.txt
    ```
...
*   EQAPO GUI will automatically attempt to fix file permissions if it encounters an error. Check the terminal logs in Dev mode for details.

**"Audio Not Changing"**
*   Ensure EqualizerAPO is installed for your specific audio device (Configurator.exe).
*   Ensure the `Include:` line in EqualizerAPO points exactly to the path shown in EQAPO GUI settings.
